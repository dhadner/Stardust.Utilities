...
{
    /// <summary>
    /// Used to support PropertyGrid editing of <see cref="UInt256"/>.
    /// Accepts decimal (culture-aware), hex "0x…", and binary "0b…" input with optional
    /// '_' digit separators and surrounding whitespace. Emits values as culture-aware
    /// decimal strings for output (matching <see cref="System.ComponentModel.Int32Converter"/>
    /// and the other BCL numeric converters). Supports the designer via
    /// <see cref="InstanceDescriptor"/>.
    /// </summary>
    public class UInt256TypeConverter : TypeConverter
    {
        private static readonly StandardValuesCollection STANDARD_VALUES =
            new(new UInt256[] { UInt256.Zero, UInt256.MaxValue });
        ...
        /// <summary>
        /// Converts the given value to a <see cref="UInt256"/> instance.
        /// </summary>
        /// <param name="context">Context information.</param>
        /// <param name="culture">Culture information (used for decimal parsing).</param>
        /// <param name="value">The value to convert.</param>
        /// <returns>The converted <see cref="UInt256"/>.</returns>
        public override object? ConvertFrom(ITypeDescriptorContext? context, CultureInfo? culture, object value)
        {
            if (value is string s)
            {
                return ConverterParsing.Parse<UInt256>(s, culture,
                    (d, p) => UInt256.Parse(d, p),
                    h => UInt256.Parse(h, NumberStyles.HexNumber));
            }
                                          ...
                                          object? value, Type destinationType)
        {
            if (destinationType == typeof(string))
            {
                ...
                if (value is UInt256 v) return v.ToString("G", culture ?? CultureInfo.CurrentCulture);
            }
            if (destinationType == typeof(InstanceDescriptor) && value is UInt256 v3)
            {
                ConstructorInfo ctor = typeof(UInt256).GetConstructor(
        ...
        public override bool IsValid(ITypeDescriptorContext? context, object? value)
        {
            ...
            if (value is string s)
            {
                return ConverterParsing.TryParse<UInt256>(s, null,
                    (d, p) => UInt256.Parse(d, p),
                    h => UInt256.Parse(h, NumberStyles.HexNumber),
                    out _);
            }
