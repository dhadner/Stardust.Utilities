...
{
    /// <summary>
    /// Used to support PropertyGrid editing of <see cref="Int32Be"/>.
    ...
    public class Int32BeTypeConverter : TypeConverter
    {
        private static readonly StandardValuesCollection STANDARD_VALUES =
            new(new Int32Be[] { new(int.MinValue), new(0), new(int.MaxValue) });
        ...
        /// <summary>
        /// Converts the given value to an <see cref="Int32Be"/> instance.
        /// </summary>
        /// <param name="context">Context information.</param>
        /// <param name="culture">Culture information (used for decimal parsing).</param>
        /// <param name="value">The value to convert.</param>
        /// <returns>The converted <see cref="Int32Be"/>.</returns>
        public override object? ConvertFrom(ITypeDescriptorContext? context, CultureInfo? culture, object value)
        {
            if (value is string s)
            {
                return ConverterParsing.Parse<Int32Be>(s, culture,
                    (d, p) => Int32Be.Parse(d, p),
                    h => Int32Be.Parse(h, NumberStyles.HexNumber));
            }
            if (value is int i) return new Int32Be(i);
                                          ...
                                          object? value, Type destinationType)
        {
            if (destinationType == typeof(string))
            {
                ...
                if (value is Int32Be v) return $"0x{(uint)(int)v:x8}";
            }
            if (destinationType == typeof(int) && value is Int32Be v2) return (int)v2;
            if (destinationType == typeof(InstanceDescriptor) && value is Int32Be v3)
            {
                ConstructorInfo ctor = typeof(Int32Be).GetConstructor(new[] { typeof(int) })!;
        ...
        {
            if (value is Int32Be) return true;
            if (value is int) return true;
            if (value is string s)
            {
                return ConverterParsing.TryParse<Int32Be>(s, null,
                    (d, p) => Int32Be.Parse(d, p),
                    h => Int32Be.Parse(h, NumberStyles.HexNumber),
                    out _);
            }
