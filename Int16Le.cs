...
namespace Stardust.Utilities
{
                             ...
                             IFormattable, ISpanFormattable, IParsable<Int16Le>, ISpanParsable<Int16Le>
    {
        ...
        /// <summary>
        /// Initializes a new <see cref="Int16Le"/> from a read-only byte span at the given offset.
        /// </summary>
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public Int16Le(ReadOnlySpan<byte> bytes, int offset, bool isBigEndian = false)
        {
        ...
        /// <summary>Initializes a new <see cref="Int16Le"/> from a byte array whose byte order is specified.</summary>
        ...
        public Int16Le(byte[] bytes, bool isBigEndian = false) : this(new ReadOnlySpan<byte>(bytes), isBigEndian) { }
        ...
        public Int16Le(byte[] bytes, int offset, bool isBigEndian = false) : this(new ReadOnlySpan<byte>(bytes), offset, isBigEndian) { }
        ...
        {
            if (destination.Length - offset < 2) throw new ArgumentException("Destination span must have at least 2 bytes", nameof(destination));
            if (isBigEndian)
            {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static Int16Le ReadFrom(ReadOnlySpan<byte> source, int offset = 0, bool isBigEndian = false) => new(source, offset, isBigEndian);

        /// <summary>Reads an <see cref="Int16Le"/> from a byte array in the specified byte order.</summary>
        /// <param name="source">A byte array containing at least 2 bytes.</param>
        /// <param name="offset">The zero-based offset into <paramref name="source"/>.</param>
        /// <param name="isBigEndian">If <see langword="false"/> (default) the source is interpreted as little-endian; if <see langword="true"/> it is interpreted as big-endian.</param>
        /// <returns>The value read from the array.</returns>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static Int16Le ReadFrom(byte[] source, int offset = 0, bool isBigEndian = false) => new(new ReadOnlySpan<byte>(source), offset, isBigEndian);
