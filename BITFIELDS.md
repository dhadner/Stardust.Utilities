...
| `BitOrder.LsbFirst` | Yes | Bit 0 = least significant | Hardware datasheets, x86 convention |
| `BitOrder.MsbFirst` | No | Bit 0 = most significant | RFCs, IETF specifications |
...
`ByteOrder.NetworkEndian` and `BitOrder.MsbFirst` so that bit positions match RFC diagrams directly.
