// <auto-generated>
// This code was generated by Stardust.Generators source generator.
// </auto-generated>
#nullable enable
using System;
using System.Buffers.Binary;
using System.Runtime.CompilerServices;
using System.Text.Json;
using System.Text.Json.Serialization;
using Stardust.Utilities;

namespace Stardust.Utilities.Tests;

public partial class FloatingPointPropertyNonAlignedTests
{
    [JsonConverter(typeof(OddAlignedViewJsonConverter))]
    public partial record struct OddAlignedView
    {
        private readonly Memory<byte> __data;
        private readonly byte __bitOffset;

        /// <summary>Minimum number of bytes required in the backing buffer.</summary>
        public const int SIZE_IN_BYTES = 23;
        /// <summary>Total number of bits in this view.</summary>
        public const int BIT_WIDTH = 128;

        /// <summary>Creates a view over the specified memory buffer.</summary>
        /// <param name="data">The buffer to view. Must contain at least <see cref="SIZE_IN_BYTES"/> bytes.</param>
        /// <exception cref="ArgumentException">The buffer is too short.</exception>
        public OddAlignedView(Memory<byte> data)
        {
            if (data.Length < SIZE_IN_BYTES)
                throw new ArgumentException($"Buffer must contain at least {SIZE_IN_BYTES} bytes, but was {data.Length}.", nameof(data));
            __data = data;
            __bitOffset = 0;
        }

        /// <summary>Creates a view over the specified byte array.</summary>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public OddAlignedView(byte[] data) : this(data.AsMemory()) { }

        /// <summary>Creates a view over a portion of the specified byte array.</summary>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public OddAlignedView(byte[] data, int offset) : this(data.AsMemory(offset)) { }

        /// <summary>Creates a sub-view at a bit offset within the specified memory buffer (used by nested views).</summary>
        internal OddAlignedView(Memory<byte> data, int bitOffset)
        {
            __data = data;
            __bitOffset = (byte)bitOffset;
        }

        /// <summary>Gets the underlying memory buffer.</summary>
        public Memory<byte> Data => __data;

        public partial byte LowPad
        {
            [MethodImpl(MethodImplOptions.AggressiveInlining)]
            get
            {
                var s = __data.Span;
                if (__bitOffset == 0)
                {
                    return (byte)(s[0] & 0x07);
                }
                int ep = 0 + __bitOffset;
                int bi = ep >> 3;
                int sh = ep & 7;
                return (byte)((BinaryPrimitives.ReadUInt16LittleEndian(s.Slice(bi)) >> sh) & 0x0007);
            }
            [MethodImpl(MethodImplOptions.AggressiveInlining)]
            set
            {
                var s = __data.Span;
                if (__bitOffset == 0)
                {
                    s[0] = (byte)((s[0] & 0xF8) | ((byte)value & 0x07));
                }
                else
                {
                    int ep = 0 + __bitOffset;
                    int bi = ep >> 3;
                    int sh = ep & 7;
                    var slice = s.Slice(bi);
                    ushort raw = BinaryPrimitives.ReadUInt16LittleEndian(slice);
                    ushort m = (ushort)(0x0007 << sh);
                    raw = (ushort)((raw & (ushort)~m) | (((ushort)value << sh) & m));
                    BinaryPrimitives.WriteUInt16LittleEndian(slice, raw);
                }
            }
        }

        public partial global::System.Half HalfVal
        {
            [MethodImpl(MethodImplOptions.AggressiveInlining)]
            get
            {
                var s = __data.Span;
                if (__bitOffset == 0)
                {
                    return BitConverter.UInt16BitsToHalf((ushort)((BinaryPrimitives.ReadUInt32LittleEndian(s.Slice(0)) >> 3) & 0xFFFFU));
                }
                int ep = 3 + __bitOffset;
                int bi = ep >> 3;
                int sh = ep & 7;
                return BitConverter.UInt16BitsToHalf((ushort)((BinaryPrimitives.ReadUInt32LittleEndian(s.Slice(bi)) >> sh) & 0xFFFFU));
            }
            [MethodImpl(MethodImplOptions.AggressiveInlining)]
            set
            {
                var s = __data.Span;
                if (__bitOffset == 0)
                {
                    var slice = s.Slice(0);
                    uint raw = BinaryPrimitives.ReadUInt32LittleEndian(slice);
                    raw = (uint)((raw & 0xFFF80007U) | (((uint)BitConverter.HalfToUInt16Bits(value) << 3) & 0x7FFF8U));
                    BinaryPrimitives.WriteUInt32LittleEndian(slice, raw);
                }
                else
                {
                    int ep = 3 + __bitOffset;
                    int bi = ep >> 3;
                    int sh = ep & 7;
                    var slice = s.Slice(bi);
                    uint raw = BinaryPrimitives.ReadUInt32LittleEndian(slice);
                    uint m = (uint)(0xFFFFU << sh);
                    raw = (uint)((raw & (uint)~m) | (((uint)BitConverter.HalfToUInt16Bits(value) << sh) & m));
                    BinaryPrimitives.WriteUInt32LittleEndian(slice, raw);
                }
            }
        }

        public partial byte MidPad1
        {
            [MethodImpl(MethodImplOptions.AggressiveInlining)]
            get
            {
                var s = __data.Span;
                if (__bitOffset == 0)
                {
                    return (byte)((s[2] >> 3) & 0x03);
                }
                int ep = 19 + __bitOffset;
                int bi = ep >> 3;
                int sh = ep & 7;
                return (byte)((BinaryPrimitives.ReadUInt16LittleEndian(s.Slice(bi)) >> sh) & 0x0003);
            }
            [MethodImpl(MethodImplOptions.AggressiveInlining)]
            set
            {
                var s = __data.Span;
                if (__bitOffset == 0)
                {
                    s[2] = (byte)((s[2] & 0xE7) | (((byte)value << 3) & 0x18));
                }
                else
                {
                    int ep = 19 + __bitOffset;
                    int bi = ep >> 3;
                    int sh = ep & 7;
                    var slice = s.Slice(bi);
                    ushort raw = BinaryPrimitives.ReadUInt16LittleEndian(slice);
                    ushort m = (ushort)(0x0003 << sh);
                    raw = (ushort)((raw & (ushort)~m) | (((ushort)value << sh) & m));
                    BinaryPrimitives.WriteUInt16LittleEndian(slice, raw);
                }
            }
        }

        public partial float FloatVal
        {
            [MethodImpl(MethodImplOptions.AggressiveInlining)]
            get
            {
                var s = __data.Span;
                if (__bitOffset == 0)
                {
                    return BitConverter.UInt32BitsToSingle((uint)((BinaryPrimitives.ReadUInt64LittleEndian(s.Slice(2)) >> 5) & 0xFFFFFFFFUL));
                }
                int ep = 21 + __bitOffset;
                int bi = ep >> 3;
                int sh = ep & 7;
                return BitConverter.UInt32BitsToSingle((uint)((BinaryPrimitives.ReadUInt64LittleEndian(s.Slice(bi)) >> sh) & 0xFFFFFFFFUL));
            }
            [MethodImpl(MethodImplOptions.AggressiveInlining)]
            set
            {
                var s = __data.Span;
                if (__bitOffset == 0)
                {
                    var slice = s.Slice(2);
                    ulong raw = BinaryPrimitives.ReadUInt64LittleEndian(slice);
                    raw = (ulong)((raw & 0xFFFFFFE00000001FUL) | (((ulong)BitConverter.SingleToUInt32Bits(value) << 5) & 0x1FFFFFFFE0UL));
                    BinaryPrimitives.WriteUInt64LittleEndian(slice, raw);
                }
                else
                {
                    int ep = 21 + __bitOffset;
                    int bi = ep >> 3;
                    int sh = ep & 7;
                    var slice = s.Slice(bi);
                    ulong raw = BinaryPrimitives.ReadUInt64LittleEndian(slice);
                    ulong m = (ulong)(0xFFFFFFFFUL << sh);
                    raw = (ulong)((raw & (ulong)~m) | (((ulong)BitConverter.SingleToUInt32Bits(value) << sh) & m));
                    BinaryPrimitives.WriteUInt64LittleEndian(slice, raw);
                }
            }
        }

        public partial byte MidPad2
        {
            [MethodImpl(MethodImplOptions.AggressiveInlining)]
            get
            {
                var s = __data.Span;
                if (__bitOffset == 0)
                {
                    return (byte)((BinaryPrimitives.ReadUInt16LittleEndian(s.Slice(6)) >> 5) & 0x001F);
                }
                int ep = 53 + __bitOffset;
                int bi = ep >> 3;
                int sh = ep & 7;
                return (byte)((BinaryPrimitives.ReadUInt16LittleEndian(s.Slice(bi)) >> sh) & 0x001F);
            }
            [MethodImpl(MethodImplOptions.AggressiveInlining)]
            set
            {
                var s = __data.Span;
                if (__bitOffset == 0)
                {
                    var slice = s.Slice(6);
                    ushort raw = BinaryPrimitives.ReadUInt16LittleEndian(slice);
                    raw = (ushort)((raw & 0xFC1F) | (((ushort)value << 5) & 0x03E0));
                    BinaryPrimitives.WriteUInt16LittleEndian(slice, raw);
                }
                else
                {
                    int ep = 53 + __bitOffset;
                    int bi = ep >> 3;
                    int sh = ep & 7;
                    var slice = s.Slice(bi);
                    ushort raw = BinaryPrimitives.ReadUInt16LittleEndian(slice);
                    ushort m = (ushort)(0x001F << sh);
                    raw = (ushort)((raw & (ushort)~m) | (((ushort)value << sh) & m));
                    BinaryPrimitives.WriteUInt16LittleEndian(slice, raw);
                }
            }
        }

        public partial double DoubleVal
        {
            [MethodImpl(MethodImplOptions.AggressiveInlining)]
            get
            {
                var s = __data.Span;
                if (__bitOffset == 0)
                {
                    return BitConverter.UInt64BitsToDouble((ulong)((BinaryPrimitives.ReadUInt128LittleEndian(s.Slice(7)) >> 2) & (UInt128)0xFFFFFFFFFFFFFFFFUL));
                }
                int ep = 58 + __bitOffset;
                int bi = ep >> 3;
                int sh = ep & 7;
                return BitConverter.UInt64BitsToDouble((ulong)((BinaryPrimitives.ReadUInt128LittleEndian(s.Slice(bi)) >> sh) & (UInt128)0xFFFFFFFFFFFFFFFFUL));
            }
            [MethodImpl(MethodImplOptions.AggressiveInlining)]
            set
            {
                var s = __data.Span;
                if (__bitOffset == 0)
                {
                    var slice = s.Slice(7);
                    UInt128 raw = BinaryPrimitives.ReadUInt128LittleEndian(slice);
                    raw = (UInt128)((raw & ~((UInt128)0xFFFFFFFFFFFFFFFFUL << 2)) | (((UInt128)BitConverter.DoubleToUInt64Bits(value) << 2) & ((UInt128)0xFFFFFFFFFFFFFFFFUL << 2)));
                    BinaryPrimitives.WriteUInt128LittleEndian(slice, raw);
                }
                else
                {
                    int ep = 58 + __bitOffset;
                    int bi = ep >> 3;
                    int sh = ep & 7;
                    var slice = s.Slice(bi);
                    UInt128 raw = BinaryPrimitives.ReadUInt128LittleEndian(slice);
                    UInt128 m = (UInt128)((UInt128)0xFFFFFFFFFFFFFFFFUL << sh);
                    raw = (UInt128)((raw & (UInt128)~m) | (((UInt128)BitConverter.DoubleToUInt64Bits(value) << sh) & m));
                    BinaryPrimitives.WriteUInt128LittleEndian(slice, raw);
                }
            }
        }

        public partial byte TopPad
        {
            [MethodImpl(MethodImplOptions.AggressiveInlining)]
            get
            {
                var s = __data.Span;
                if (__bitOffset == 0)
                {
                    return (byte)((s[15] >> 2) & 0x3F);
                }
                int ep = 122 + __bitOffset;
                int bi = ep >> 3;
                int sh = ep & 7;
                return (byte)((BinaryPrimitives.ReadUInt16LittleEndian(s.Slice(bi)) >> sh) & 0x003F);
            }
            [MethodImpl(MethodImplOptions.AggressiveInlining)]
            set
            {
                var s = __data.Span;
                if (__bitOffset == 0)
                {
                    s[15] = (byte)((s[15] & 0x03) | (((byte)value << 2) & 0xFC));
                }
                else
                {
                    int ep = 122 + __bitOffset;
                    int bi = ep >> 3;
                    int sh = ep & 7;
                    var slice = s.Slice(bi);
                    ushort raw = BinaryPrimitives.ReadUInt16LittleEndian(slice);
                    ushort m = (ushort)(0x003F << sh);
                    raw = (ushort)((raw & (ushort)~m) | (((ushort)value << sh) & m));
                    BinaryPrimitives.WriteUInt16LittleEndian(slice, raw);
                }
            }
        }

        /// <summary>Optional description (title) for this view.</summary>
        public static string? StructDescription => null;
        /// <summary>Optional resource type for the struct description.</summary>
        public static Type? StructDescriptionResourceType => null;
        /// <summary>Metadata for every field and flag declared on this view, in declaration order.</summary>
        public static ReadOnlySpan<BitFieldInfo> Fields => new BitFieldInfo[]
        {
            new("LowPad", 0, 3, "byte", false, ByteOrder.LittleEndian, BitOrder.BitZeroIsLsb, StructTotalBits: 184, FieldMustBe: 0),
            new("HalfVal", 3, 16, "System.Half", false, ByteOrder.LittleEndian, BitOrder.BitZeroIsLsb, StructTotalBits: 184, FieldMustBe: 0),
            new("MidPad1", 19, 2, "byte", false, ByteOrder.LittleEndian, BitOrder.BitZeroIsLsb, StructTotalBits: 184, FieldMustBe: 0),
            new("FloatVal", 21, 32, "float", false, ByteOrder.LittleEndian, BitOrder.BitZeroIsLsb, StructTotalBits: 184, FieldMustBe: 0),
            new("MidPad2", 53, 5, "byte", false, ByteOrder.LittleEndian, BitOrder.BitZeroIsLsb, StructTotalBits: 184, FieldMustBe: 0),
            new("DoubleVal", 58, 64, "double", false, ByteOrder.LittleEndian, BitOrder.BitZeroIsLsb, StructTotalBits: 184, FieldMustBe: 0),
            new("TopPad", 122, 6, "byte", false, ByteOrder.LittleEndian, BitOrder.BitZeroIsLsb, StructTotalBits: 184, FieldMustBe: 0),
        };

        private sealed class OddAlignedViewJsonConverter : JsonConverter<OddAlignedView>
        {
            /// <summary>Reads a OddAlignedView from a JSON hex string.</summary>
            public override OddAlignedView Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
            {
                var s = reader.GetString();
                if (s is null) return new OddAlignedView(new byte[SIZE_IN_BYTES]);
                ReadOnlySpan<char> hex = s.AsSpan();
                if (hex.Length >= 2 && hex[0] == '0' && (hex[1] == 'x' || hex[1] == 'X'))
                    hex = hex.Slice(2);
                var bytes = new byte[SIZE_IN_BYTES];
                int hexLen = hex.Length;
                for (int i = 0; i < SIZE_IN_BYTES && (hexLen - i * 2) > 0; i++)
                {
                    int hi = hexLen - (i + 1) * 2;
                    if (hi >= 0)
                        bytes[i] = (byte)((HexVal(hex[hi]) << 4) | HexVal(hex[hi + 1]));
                    else if (hi == -1)
                        bytes[i] = (byte)HexVal(hex[0]);
                }
                return new OddAlignedView(bytes);
            }

            /// <summary>Writes a OddAlignedView to JSON as a hex string.</summary>
            public override void Write(Utf8JsonWriter writer, OddAlignedView value, JsonSerializerOptions options)
            {
                var s = value.__data.Span;
                // Find highest non-zero byte for minimal hex output
                int top = SIZE_IN_BYTES - 1;
                while (top > 0 && s[top] == 0) top--;
                // Build hex string from most-significant to least-significant byte
                var chars = new char[2 + (top + 1) * 2];
                chars[0] = '0';
                chars[1] = 'x';
                const string HEX_CHARS = "0123456789ABCDEF";
                // First byte (top) may have leading zero nibble -- include both for simplicity
                for (int i = top; i >= 0; i--)
                {
                    int pos = 2 + (top - i) * 2;
                    chars[pos] = HEX_CHARS[s[i] >> 4];
                    chars[pos + 1] = HEX_CHARS[s[i] & 0xF];
                }
                writer.WriteStringValue(new string(chars));
            }

            [MethodImpl(MethodImplOptions.AggressiveInlining)]
            private static int HexVal(char c) => c <= '9' ? c - '0' : (c <= 'F' ? c - 'A' + 10 : c - 'a' + 10);
        }

    }
}
