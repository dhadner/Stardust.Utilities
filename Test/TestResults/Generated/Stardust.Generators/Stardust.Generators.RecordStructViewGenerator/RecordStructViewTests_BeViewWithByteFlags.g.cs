...
{
    [JsonConverter(typeof(BeViewWithByteFlagsJsonConverter))]
    public partial record struct BeViewWithByteFlags
    {
        ...
        public partial global::Stardust.Utilities.Tests.StatusFlags Flags
        {
            ...
            get
            {
                ...
                if (__bitOffset == 0)
                {
                    return (global::Stardust.Utilities.Tests.StatusFlags)(byte)s[0];
                }
                int ep = 0 + __bitOffset;
                int bi = ep >> 3;
                int endInWindow = (ep + 7) - bi * 8;
                int sh = 16 - 1 - endInWindow;
                return (global::Stardust.Utilities.Tests.StatusFlags)(byte)((BinaryPrimitives.ReadUInt16LittleEndian(s.Slice(bi)) >> sh) & 0x00FF);
            }
            [MethodImpl(MethodImplOptions.AggressiveInlining)]
            set
            {
        ...
        private sealed class BeViewWithByteFlagsJsonConverter : JsonConverter<BeViewWithByteFlags>
        {
            /// <summary>Reads a BeViewWithByteFlags from a JSON hex string.</summary>
            public override BeViewWithByteFlags Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
            {
                var s = reader.GetString();
                if (s is null) return new BeViewWithByteFlags(new byte[SIZE_IN_BYTES]);
                ...
                return new BeViewWithByteFlags(bytes);
            }

            /// <summary>Writes a BeViewWithByteFlags to JSON as a hex string.</summary>
            public override void Write(Utf8JsonWriter writer, BeViewWithByteFlags value, JsonSerializerOptions options)
            {
