...
{
    [JsonConverter(typeof(ViewWithTruncatedHeaderJsonConverter))]
    public partial record struct ViewWithTruncatedHeader
    {
        ...
        public partial global::Stardust.Utilities.Tests.ProtocolHeader16 TruncatedHeader
        {
            [MethodImpl(MethodImplOptions.AggressiveInlining)]
            get
            {
                ...
                if (__bitOffset == 0)
                {
                    return (global::Stardust.Utilities.Tests.ProtocolHeader16)(ushort)s[0];
                }
                int ep = 0 + __bitOffset;
                int bi = ep >> 3;
                int sh = ep & 7;
                return (global::Stardust.Utilities.Tests.ProtocolHeader16)(ushort)((BinaryPrimitives.ReadUInt16LittleEndian(s.Slice(bi)) >> sh) & 0x00FF);
            }
            [MethodImpl(MethodImplOptions.AggressiveInlining)]
            set
            {
        ...
        {
            new("TruncatedHeader", 0, 8, "Stardust.Utilities.Tests.ProtocolHeader16", false, ByteOrder.LittleEndian, BitOrder.BitZeroIsLsb, StructTotalBits: 16, FieldMustBe: 0),
            new("Other", 8, 8, "byte", false, ByteOrder.LittleEndian, BitOrder.BitZeroIsLsb, StructTotalBits: 16, FieldMustBe: 0),
        };

        private sealed class ViewWithTruncatedHeaderJsonConverter : JsonConverter<ViewWithTruncatedHeader>
        {
            /// <summary>Reads a ViewWithTruncatedHeader from a JSON hex string.</summary>
            public override ViewWithTruncatedHeader Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
            {
                var s = reader.GetString();
                if (s is null) return new ViewWithTruncatedHeader(new byte[SIZE_IN_BYTES]);
                ...
                return new ViewWithTruncatedHeader(bytes);
            }

            /// <summary>Writes a ViewWithTruncatedHeader to JSON as a hex string.</summary>
            public override void Write(Utf8JsonWriter writer, ViewWithTruncatedHeader value, JsonSerializerOptions options)
            {
