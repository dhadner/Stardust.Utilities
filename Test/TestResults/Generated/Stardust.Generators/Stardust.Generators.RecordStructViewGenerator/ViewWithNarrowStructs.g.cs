...
public partial record struct ViewWithNarrowStructs
{
    ...
    public ViewWithNarrowStructs(Memory<byte> data)
    {
    ...
    public ViewWithNarrowStructs(byte[] data) : this(data.AsMemory()) { }

    /// <summary>Creates a view over a portion of the specified byte array.</summary>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public ViewWithNarrowStructs(byte[] data, int offset) : this(data.AsMemory(offset)) { }
    ...
    public partial global::Stardust.Utilities.Tests.SensorReading12 Sensor
    {
        ...
        set
        {
            ...
            if (__bitOffset == 0)
            {
                var slice = s.Slice(1);
                ushort raw = BinaryPrimitives.ReadUInt16LittleEndian(slice);
                raw = (ushort)((raw & 0xF000) | ((ushort)(ushort)value & 0x0FFF));
                BinaryPrimitives.WriteUInt16LittleEndian(slice, raw);
            }
            else
            {
    ...
    public partial global::Stardust.Utilities.Tests.RgbColor24 Color
    {
        ...
        set
        {
            ...
            if (__bitOffset == 0)
            {
                var slice = s.Slice(3);
                uint raw = BinaryPrimitives.ReadUInt32LittleEndian(slice);
                raw = (uint)((raw & 0xFF000000U) | ((uint)(uint)value & 0xFFFFFFU));
                BinaryPrimitives.WriteUInt32LittleEndian(slice, raw);
            }
            else
            {
    ...
    {
        new("Status", 0, 5, "Stardust.Utilities.Tests.StatusCode5", false, ByteOrder.LittleEndian, BitOrder.BitZeroIsLsb, StructTotalBits: 56, FieldMustBe: 0),
        new("Sensor", 8, 12, "Stardust.Utilities.Tests.SensorReading12", false, ByteOrder.LittleEndian, BitOrder.BitZeroIsLsb, StructTotalBits: 56, FieldMustBe: 0),
        new("Color", 24, 24, "Stardust.Utilities.Tests.RgbColor24", false, ByteOrder.LittleEndian, BitOrder.BitZeroIsLsb, StructTotalBits: 56, FieldMustBe: 0),
        new("Tag", 48, 8, "byte", false, ByteOrder.LittleEndian, BitOrder.BitZeroIsLsb, StructTotalBits: 56, FieldMustBe: 0),
    };

    private sealed class ViewWithNarrowStructsJsonConverter : JsonConverter<ViewWithNarrowStructs>
    {
        /// <summary>Reads a ViewWithNarrowStructs from a JSON hex string.</summary>
        public override ViewWithNarrowStructs Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
        {
            var s = reader.GetString();
            if (s is null) return new ViewWithNarrowStructs(new byte[SIZE_IN_BYTES]);
            ...
            return new ViewWithNarrowStructs(bytes);
        }
