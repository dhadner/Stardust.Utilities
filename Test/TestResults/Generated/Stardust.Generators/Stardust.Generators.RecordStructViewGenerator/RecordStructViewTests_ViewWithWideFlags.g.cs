...
{
    [JsonConverter(typeof(ViewWithWideFlagsJsonConverter))]
    public partial record struct ViewWithWideFlags
    {
        ...
        public ViewWithWideFlags(Memory<byte> data)
        {
        ...
        public ViewWithWideFlags(byte[] data) : this(data.AsMemory()) { }

        /// <summary>Creates a view over a portion of the specified byte array.</summary>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public ViewWithWideFlags(byte[] data, int offset) : this(data.AsMemory(offset)) { }

        /// <summary>Creates a sub-view at a bit offset within the specified memory buffer (used by nested views).</summary>
        internal ViewWithWideFlags(Memory<byte> data, int bitOffset)
        {
            _data = data;
        ...
        {
            new("WideFlags", 0, 16, "Stardust.Utilities.Tests.StatusFlags", false, ByteOrder.LittleEndian, BitOrder.BitZeroIsMsb, StructTotalBits: 24, FieldMustBe: 0),
            new("Payload", 16, 8, "byte", false, ByteOrder.BigEndian, BitOrder.BitZeroIsMsb, StructTotalBits: 24, FieldMustBe: 0),
        };

        private sealed class ViewWithWideFlagsJsonConverter : JsonConverter<ViewWithWideFlags>
        {
            /// <summary>Reads a ViewWithWideFlags from a JSON hex string.</summary>
            public override ViewWithWideFlags Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
            {
                var s = reader.GetString();
                if (s is null) return new ViewWithWideFlags(new byte[SIZE_IN_BYTES]);
                ...
                return new ViewWithWideFlags(bytes);
            }

            /// <summary>Writes a ViewWithWideFlags to JSON as a hex string.</summary>
            public override void Write(Utf8JsonWriter writer, ViewWithWideFlags value, JsonSerializerOptions options)
            {
