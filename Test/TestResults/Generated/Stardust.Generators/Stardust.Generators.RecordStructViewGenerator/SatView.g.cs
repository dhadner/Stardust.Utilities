...
public partial record struct SatView
{
    ...
    public SatView(Memory<byte> data)
    {
    ...
    public SatView(byte[] data) : this(data.AsMemory()) { }

    /// <summary>Creates a view over a portion of the specified byte array.</summary>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public SatView(byte[] data, int offset) : this(data.AsMemory(offset)) { }
    ...
    {
        new("Clamped", 0, 5, "byte", false, ByteOrder.LittleEndian, BitOrder.BitZeroIsLsb, StructTotalBits: 8, FieldMustBe: 0),
        new("Other", 5, 3, "byte", false, ByteOrder.LittleEndian, BitOrder.BitZeroIsLsb, StructTotalBits: 8, FieldMustBe: 0),
    };

    private sealed class SatViewJsonConverter : JsonConverter<SatView>
    {
        /// <summary>Reads a SatView from a JSON hex string.</summary>
        public override SatView Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
        {
            var s = reader.GetString();
            if (s is null) return new SatView(new byte[SIZE_IN_BYTES]);
