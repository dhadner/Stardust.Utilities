...
{
    /// <summary>
    /// Used to support PropertyGrid editing of <see cref="UInt16Le"/>.
    ...
    /// </summary>
    public class UInt16LeTypeConverter : TypeConverter
    {
        private static readonly StandardValuesCollection STANDARD_VALUES =
            new(new UInt16Le[] { new((ushort)0), new(ushort.MaxValue) });
        ...
        /// <summary>
        /// Converts the given value to a <see cref="UInt16Le"/> instance.
        /// </summary>
        /// <param name="context">Context information.</param>
        /// <param name="culture">Culture information (used for decimal parsing).</param>
        /// <param name="value">The value to convert.</param>
        /// <returns>The converted <see cref="UInt16Le"/>.</returns>
        public override object? ConvertFrom(ITypeDescriptorContext? context, CultureInfo? culture, object value)
        {
            if (value is string s)
            {
                return ConverterParsing.Parse<UInt16Le>(s, culture,
                    (d, p) => UInt16Le.Parse(d, p),
                    h => UInt16Le.Parse(h, NumberStyles.HexNumber));
            }
            if (value is ushort u) return new UInt16Le(u);
                                          ...
                                          object? value, Type destinationType)
        {
            if (destinationType == typeof(string))
            {
                ...
                if (value is UInt16Le v) return $"0x{(ushort)v:x4}";
            }
            if (destinationType == typeof(ushort) && value is UInt16Le v2) return (ushort)v2;
            if (destinationType == typeof(InstanceDescriptor) && value is UInt16Le v3)
            {
                ConstructorInfo ctor = typeof(UInt16Le).GetConstructor(new[] { typeof(ushort) })!;
        ...
        {
            if (value is UInt16Le) return true;
            if (value is ushort) return true;
            if (value is string s)
            {
                return ConverterParsing.TryParse<UInt16Le>(s, null,
                    (d, p) => UInt16Le.Parse(d, p),
                    h => UInt16Le.Parse(h, NumberStyles.HexNumber),
                    out _);
            }
