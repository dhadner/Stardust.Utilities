...
{
    /// <summary>
    /// Used to support PropertyGrid editing of <see cref="UInt64Le"/>.
    ...
    /// </summary>
    public class UInt64LeTypeConverter : TypeConverter
    {
        private static readonly StandardValuesCollection STANDARD_VALUES =
            new(new UInt64Le[] { new(0UL), new(ulong.MaxValue) });
        ...
        /// <summary>
        /// Converts the given value to a <see cref="UInt64Le"/> instance.
        /// </summary>
        /// <param name="context">Context information.</param>
        /// <param name="culture">Culture information (used for decimal parsing).</param>
        /// <param name="value">The value to convert.</param>
        /// <returns>The converted <see cref="UInt64Le"/>.</returns>
        public override object? ConvertFrom(ITypeDescriptorContext? context, CultureInfo? culture, object value)
        {
            if (value is string s)
            {
                return ConverterParsing.Parse<UInt64Le>(s, culture,
                    (d, p) => UInt64Le.Parse(d, p),
                    h => UInt64Le.Parse(h, NumberStyles.HexNumber));
            }
            if (value is ulong u) return new UInt64Le(u);
                                          ...
                                          object? value, Type destinationType)
        {
            if (destinationType == typeof(string))
            {
                ...
                if (value is UInt64Le v) return $"0x{(ulong)v:x16}";
            }
            if (destinationType == typeof(ulong) && value is UInt64Le v2) return (ulong)v2;
            if (destinationType == typeof(InstanceDescriptor) && value is UInt64Le v3)
            {
                ConstructorInfo ctor = typeof(UInt64Le).GetConstructor(new[] { typeof(ulong) })!;
        ...
        {
            if (value is UInt64Le) return true;
            if (value is ulong) return true;
            if (value is string s)
            {
                return ConverterParsing.TryParse<UInt64Le>(s, null,
                    (d, p) => UInt64Le.Parse(d, p),
                    h => UInt64Le.Parse(h, NumberStyles.HexNumber),
                    out _);
            }
