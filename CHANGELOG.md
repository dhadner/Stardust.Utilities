...
- **`BitOrder` enum** (`MsbFirst`, `LsbFirst`) for controlling bit numbering in `[BitFields]` and `[BitFieldsView]`.
...
- **`bitOrder` and `byteOrder` optional parameters** on `[BitFields]` attribute constructors. Defaults (`LsbFirst`, `LittleEndian`) preserve backwards compatibility.
