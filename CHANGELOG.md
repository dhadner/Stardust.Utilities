...
### 🟢 Added
- **`Default` static property on all value-type structs** -- every `[BitFields]` value-type struct (single-word and multi-word) now generates a `public static {TypeName} Default => default;` property. For unconstrained types, `Default` returns all-zero bits. For types with `MustBe` or `UndefinedBitsMustBe` constraints, `Default` is safe to use because all property reads go through the `__normalizedValue` path, which applies the `__NORMALIZATION_AND_MASK` and `__NORMALIZATION_OR_MASK` at every exit point -- construction, operators, `With` methods, and property getters. This means `Default` always presents the correct constrained view: `MustBe.One` flags read as `true`, `MustBe.Zero` fields read as `0`, and undefined bits obey their policy. `Default` is the recommended starting point for fluent builder patterns: `MyReg.Default.WithMode(5).WithReady(true)`. Includes 7 tests covering unconstrained 8/16/32/64-bit types, `MustBe` normalization, combined `MustBe` + `UndefinedBitsMustBe` normalization, and fluent builder round-trips.
- **Auto-sized `[BitFields]` on value-type structs**

### 🟣 Changed
- **`BIT_WIDTH` constant added to all generated types** -- every `[BitFields]` type now generates a `public const int BIT_WIDTH` giving the total number of defined bits. Previously only record struct views had this constant. For auto-sized and bit-count structs, `BIT_WIDTH` can differ from `SIZE_IN_BYTES * 8` (e.g., a `[BitFields]` struct with fields spanning 5 bits has `SIZE_IN_BYTES = 1` but `BIT_WIDTH = 5`). For `nint`/`nuint` types, `BIT_WIDTH` is a `public static int` property (platform-dependent). This harmonizes the public API across all three generator modes: single-word value types, multi-word value types, and record struct views.
...
- **`BIT_WIDTH` is additive** -- the new `BIT_WIDTH` constant is a new public member. Code that already declares a member named `BIT_WIDTH` in a `partial struct` will get a compile error from the duplicate definition. This is unlikely since it was already generated on record struct views in prior versions.
...
- **`Default` is additive** -- the new `Default` static property is a new public member on all value-type structs. Code that already declares a member named `Default` in a `partial struct` will get a compile error from the duplicate definition.
