...
- **Endian API changes are source-compatible.** Adding `int offset = 0` to `WriteTo` / `TryWriteTo` / `ReadFrom` preserves existing named-argument calls (`v.WriteTo(buf, isBigEndian: false)`) and default-argument calls (`v.WriteTo(buf)`). Positional calls like `v.WriteTo(buf, false)` would no longer bind (the second positional would now be `int offset`, not `bool isBigEndian`), but the library's own tests and a scan of the MacView consumer codebase found no such call sites, and every shipped sample uses named arguments.
