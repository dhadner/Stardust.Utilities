                              ...
                              IFormattable, ISpanFormattable, IParsable<Int128Be>, ISpanParsable<Int128Be>
    {
        ...
        public readonly void ToBytes(IList<byte> bytes, int offset = 0, bool isBigEndian = true)
        {
            ...
            else
            {
                ...
                hi.WriteTo(buf);
                lo.WriteTo(buf.Slice(8));
                for (int i = 0; i < 16; i++) bytes[offset + i] = buf[15 - i];
            }
        }
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static Int128Be ReadFrom(ReadOnlySpan<byte> source, bool isBigEndian = true) => new(source, isBigEndian);
