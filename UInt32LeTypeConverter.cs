...
{
    /// <summary>
    /// Used to support PropertyGrid editing of <see cref="UInt32Le"/>.
    ...
    /// </summary>
    public class UInt32LeTypeConverter : TypeConverter
    {
        private static readonly StandardValuesCollection STANDARD_VALUES =
            new(new UInt32Le[] { new(0U), new(uint.MaxValue) });
        ...
        /// <summary>
        /// Converts the given value to a <see cref="UInt32Le"/> instance.
        /// </summary>
        /// <param name="context">Context information.</param>
        /// <param name="culture">Culture information (used for decimal parsing).</param>
        /// <param name="value">The value to convert.</param>
        /// <returns>The converted <see cref="UInt32Le"/>.</returns>
        public override object? ConvertFrom(ITypeDescriptorContext? context, CultureInfo? culture, object value)
        {
            if (value is string s)
            {
                return ConverterParsing.Parse<UInt32Le>(s, culture,
                    (d, p) => UInt32Le.Parse(d, p),
                    h => UInt32Le.Parse(h, NumberStyles.HexNumber));
            }
            if (value is uint u) return new UInt32Le(u);
                                          ...
                                          object? value, Type destinationType)
        {
            if (destinationType == typeof(string))
            {
                ...
                if (value is UInt32Le v) return $"0x{(uint)v:x8}";
            }
            if (destinationType == typeof(uint) && value is UInt32Le v2) return (uint)v2;
            if (destinationType == typeof(InstanceDescriptor) && value is UInt32Le v3)
            {
                ConstructorInfo ctor = typeof(UInt32Le).GetConstructor(new[] { typeof(uint) })!;
        ...
        {
            if (value is UInt32Le) return true;
            if (value is uint) return true;
            if (value is string s)
            {
                return ConverterParsing.TryParse<UInt32Le>(s, null,
                    (d, p) => UInt32Le.Parse(d, p),
                    h => UInt32Le.Parse(h, NumberStyles.HexNumber),
                    out _);
            }
